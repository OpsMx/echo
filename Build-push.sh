...
./gradlew --no-daemon -PenableCrossCompilerPlugin=true echo-web:installDist -x test

   # Assigning Rhel Image Name according to Quay.io Details
   IMAGENAME="quay.io/opsmxpublic/ubi8-oes-echo:${GITHASH}-${BUILD_NUMBER}"
   
   # Assigning Rhel Image Name according to Docker.io Details
   RELEASE_IMAGENAME="opsmx11/ubi8-oes-echo:${GITHASH}-${BUILD_NUMBER}"
   
   # To Build Docker image with Given Docker File
   docker build -t $IMAGENAME -t $RELEASE_IMAGENAME .  -f  ${DOCKERFILE_PATH} --no-cache  --build-arg CUSTOMPLUGIN_RELEASEVERSION=${CUSTOMPLUGIN_RELEASEVERSION} 
...
echo \"echo\": \"${IMAGENAME}\" > file.properties;
